...
plt.plot(E, Green_real, label="Re($G_{12}$)", color="red")
plt.plot(E, Green_imag, label="Im($G_{12}$)", color="blue")
...
plt.grid(True)
plt.title("$G_{12}$ for alternating chain, $t = 1$ and $\epsilon = \pm0.5$")
plt.ylabel("$G_{12}$")
plt.xlabel("Energy")
plt.legend()
plt.show()
