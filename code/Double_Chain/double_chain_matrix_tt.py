...
t = 1 # this is the binding between uper and lower site
tt = 3 # this is the binding between i and i+1
epsilon = 0

t_mat = np.array([[tt, 0], [0, tt]])
...
for value in E:
    Green_analytical = np.append(Green_analytical, analytical_Green(
        value, t))
...
plt.grid(True)
plt.title("$G_{11}$ by Iteration for $t = 1$ and $\overline{t} = 3$")
plt.ylabel("$G_{11}$")
plt.xlabel("Energy")
plt.legend()
plt.show()
