...
plt.plot(E[:79], solution_real, marker="v", label="Re($G_{11})$", markevery=2)
plt.plot(E[:79], solution_imag, marker="x", label="Im($G_{11})$", markevery=2)

plt.grid(True)
plt.legend()

plt.title("Surface Green Function for $\epsilon = 0.5$ and $t=1$")
plt.ylabel("$G_{11}$")
plt.xlabel("Energy")
plt.show()
